...
#
# `notarytool submit --wait` does exit non-zero when Apple returns Invalid, but
# all it prints is a submission id: the reasons -- the unsigned binary, the
# missing hardened runtime, the bad entitlement -- live behind
# `notarytool log <id>`, which is a second round trip nobody is at the keyboard
# to make. Fetching it here is the difference between "notarization failed" and
# a named cause, on a step that takes tens of minutes to reach a second time.
#
