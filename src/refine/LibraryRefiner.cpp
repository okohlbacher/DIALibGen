...
namespace ODIA
{
  namespace
  {
    ...
    bool fragmentIdentity(std::string_view t, ObservedFragment& f)
    {
      if (t.size() < 2 || !std::isalpha(static_cast<unsigned char>(t[0]))) { return false; }
      f.type = parseFragmentType(t.substr(0, 1));
      if (f.type == FragmentType::Unknown) { return false; }
      std::size_t i = 1;
      unsigned ordinal = 0;
      while (i < t.size() && std::isdigit(static_cast<unsigned char>(t[i])) && ordinal <= 255)
      { ordinal = ordinal * 10 + static_cast<unsigned>(t[i++] - '0'); }
      if (i == 1 || ordinal == 0 || ordinal > 255 || i >= t.size() || t[i++] != '^') { return false; }
      const std::size_t start = i;
      unsigned charge = 0;
      while (i < t.size() && std::isdigit(static_cast<unsigned char>(t[i])) && charge <= 4)
      { charge = charge * 10 + static_cast<unsigned>(t[i++] - '0'); }
      if (i == start || charge == 0 || charge > 4 || i >= t.size() || t[i++] != '/') { return false; }
      if (!number(t.substr(i), f.mz) || f.mz <= 0) { return false; }
      f.ordinal = static_cast<std::uint8_t>(ordinal);
      f.charge = static_cast<std::int8_t>(charge);
      return true;
    }
                         ...
                         std::vector<ObservedFragment>& out, std::size_t* bad_tokens)
  {
    ...
    for (std::size_t n = 0; n < fi.size(); ++n)
    {
      ObservedFragment f;
      double q = 0.0, c = NaN;
      if (!fragmentIdentity(fi[n], f) || !number(fq[n], q) || (!corr.empty() && !number(fc[n], c)) ||
          std::abs(q) > std::numeric_limits<float>::max() || (!corr.empty() && std::abs(c) > std::numeric_limits<float>::max()))
      { if (bad_tokens) { ++*bad_tokens; } continue; }
                                                          ...
                                                          RefineStats& stats)
  {
    ...
    std::set<std::string> numbered_fragments;
    if (p.write_intensity && !(finfo_c && fquant_c))
    {
      for (const auto& name : table->ColumnNames())
      {
        if (!name.starts_with("Fr.")) { continue; }
        const auto dot = name.find('.', 3);
        if (dot == std::string::npos || dot == 3 ||
            !std::all_of(name.begin() + 3, name.begin() + dot, [](unsigned char c) { return std::isdigit(c); })) { continue; }
        const auto suffix = name.substr(dot + 1);
        if (suffix == "Id" || suffix == "Quantity" || suffix == "Score") { numbered_fragments.insert(name.substr(0, dot)); }
      }
      for (const auto& prefix : numbered_fragments)
      {
        if (!table->GetColumnByName(prefix + ".Id") || !table->GetColumnByName(prefix + ".Quantity"))
        { throw std::runtime_error("incomplete DIA-NN fragment columns: " + prefix + " needs both .Id and .Quantity"); }
        if (p.intensity_min_correlation > -1.0 && !table->GetColumnByName(prefix + ".Score"))
        { throw std::runtime_error("intensity quality gate needs " + prefix + ".Score; disable explicitly with -intensity_min_correlation -1"); }
      }
    }
    ...
    if (p.write_intensity && !(finfo_c && fquant_c) && numbered_fragments.empty())
    { throw std::runtime_error("-write_intensity needs Fragment.Info/Fragment.Quant.Raw (--report-lib-info) or "
                               "DIA-NN 2 Fr.N.Id/Fr.N.Quantity (--export-quant). Refused rather than degraded to an "
                               "RT-only refinement that would pass for the treatment arm."); }
    if (p.write_intensity && numbered_fragments.empty() && !fcorr_c && p.intensity_min_correlation > -1.0)
    ...
    struct FragmentColumn
    {
      std::vector<std::string> ids;
      std::vector<double> quantity, score;
      bool has_score;
    };
    std::vector<FragmentColumn> fragment_columns;
    for (const auto& prefix : numbered_fragments)
    {
      const auto score = table->GetColumnByName(prefix + ".Score");
      fragment_columns.push_back({toStrings(table->GetColumnByName(prefix + ".Id"), rows),
        toDoubles(table->GetColumnByName(prefix + ".Quantity"), rows), toDoubles(score, rows), score != nullptr});
    }
    ...
    for (std::int64_t r = 0; r < rows; ++r)
    {
      ...
      for (const auto& f : fragment_columns)
      {
        if (f.ids[i].empty()) { continue; } // unused DIA-NN export slot
        ObservedFragment fragment;
        if (!fragmentIdentity(f.ids[i], fragment) || !finite(f.quantity[i]) ||
            std::abs(f.quantity[i]) > std::numeric_limits<float>::max() ||
            (f.has_score && (!finite(f.score[i]) || std::abs(f.score[i]) > std::numeric_limits<float>::max())))
        { ++stats.intensity_bad_tokens; continue; }
        fragment.quant = static_cast<float>(f.quantity[i]);
        fragment.correlation = static_cast<float>(f.score[i]);
        o.frags.push_back(fragment);
      }
