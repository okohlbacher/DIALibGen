...
namespace ODIA
{
  ...
  /// and Fragment.Correlations (DIA-NN 1.9), or Fr.N.Id/Quantity/Score (2.x).
  struct ObservedFragment
  {
    ...
    float quant = 0.0f;           ///< observed integrated fragment area from ONE run
    float correlation = std::numeric_limits<float>::quiet_NaN();   ///< correlation or DIA-NN 2 fragment Score; NaN = column absent
  };
  ...
  struct RefineParams
  {
    ...
    /// or Fr.N.Id/Fr.N.Quantity (--export-quant in DIA-NN 2.x) in -ids.
    bool write_intensity = false;

    /// A fragment is TRUSTED when quant > 0 and correlation (DIA-NN 1.9) or
    /// fragment Score (DIA-NN 2.x) > this. The default
