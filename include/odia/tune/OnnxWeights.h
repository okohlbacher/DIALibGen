...
namespace ODIA::tune
{
  struct OnnxFile
  {
    ...
    /// ModelProto.metadata_props, including any pending updates.
    std::map<std::string, std::string> metadata() const;
    /// Replace one metadata value on write; preserve all other protobuf fields.
    /// Does not move initializer offsets in `bytes`.
    void setMetadata(const std::string& key, const std::string& value);
    const Tensor& initializer(const std::string& name) const;
    bool hasInitializer(const std::string& name) const;

  private:
    std::map<std::string, std::string> metadata_updates_;
  };
